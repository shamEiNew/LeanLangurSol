...
/-- A stack is represented as a list of natural numbers. The first element is the *top* of the stack. -/
...
-/
def run! (p: Program) (s: Stack) : Stack := -- defines `eval!`
  match p with -- splits computation into cases by pattern matching
  | [] => s -- matches the empty list and returns `s`
  | push n :: p' => run! p' (n :: s) -- matches a program beginning with `push n` and runs the rest with `n` pushed on the stack
  | pop :: p' => -- matches a program beginning with `pop` and runs the rest after dropping the stack top
      run! p' s.tail!
  | add :: p' => -- matches a program beginning with `add` and runs the rest after replacing the top two stack entries by their sum
      ...
      run! p' ((x + y) :: s.tail!.tail!)

/--
Safe evaluation of a program using `Option`.
Returns `none` if an instruction is executed on an insufficient stack.
-/
def run? (p: Program) (s: Stack) : Option Stack := -- defines `eval?`
  match p with -- splits computation into cases by pattern matching
  | [] => some s -- matches the empty list and returns `some s`
  | push n :: p' => run? p' (n :: s) -- matches a program beginning with `push n` and safely runs the rest with `n` pushed on the stack
  | add :: p' => -- matches a program beginning with `add` and checks whether the stack has two entries to add
      ...
      | x :: y :: zs => run? p' ((x + y) :: zs) -- matches a list with at least two elements and returns `eval? p' ((x + y) :: zs)`
  ...
  | pop :: p' => -- matches a program beginning with `pop` and checks whether the stack has an entry to remove
      match s with -- splits computation into cases by pattern matching
      | _ :: ys => run? p' ys -- matches a nonempty list and returns `eval? p' ys`
      | _ => none -- matches any remaining form and returns `none`
...
inductive ValidProgram : (initStackSize : Nat) → (p : Program) → Prop -- declares the inductive type or proposition `ValidProgram`
  ...
  /-- If the first instruction is a push and the rest of the program is valid  with initial stack size `n + 1` then the program is valid with initial stack size `n` -/
  | push  {p : Program} : -- declares another constructor or syntax alternative
      ValidProgram (n + 1) p →
      ValidProgram n (push k :: p)
  /-- If the first instruction is a pop and the rest of the program is valid with initial stack size `n` then the program is valid with initial stack size `n + 1` -/
  | pop  {p : Program} : -- declares another constructor or syntax alternative
      ValidProgram n p →
      ValidProgram (n + 1) (pop :: p)
  /-- If the first instruction is an add and the rest of the program is valid with initial stack size `n + 2` then the program is valid with initial stack size `n + 1` -/
...
/-!
## Technical results about validity
-/
...
/-- Adding values on a stack of size `n+2` is valid if the rest is valid for `n+1`. -/
@[simp, grind .] -- annotation controlling elaboration, simplification, or automation
theorem valid_program_add (n: Nat) (h : ValidProgram (n + 1) p') : -- states and proves theorem `valid_program_add`
  ValidProgram (n + 2) (add :: p') := -- states the goal that an `add` program is valid from a stack with two extra entries
...
/-!
## Validity of Stack Machine Programs
...
example (a b c n : Nat) : ValidProgram n [push a, push b, add, push c, add] := by -- starts tactic mode; the goal is validity of a concrete stack program from an empty stack
  grind (ematch := 7) -- gives `grind` a larger search budget so it can chain the validity constructors

/-!
## More technical results about validity.
-/
...
@[simp] -- annotation controlling elaboration, simplification, or automation
theorem valid_program_of_add {n: Nat} (h : ValidProgram (n + 2) (add :: p')) : ValidProgram (n + 1) p' := by -- starts tactic mode; the goal extracts tail validity after an `add`
...
@[simp] -- annotation controlling elaboration, simplification, or automation
theorem valid_program_of_pop {n: Nat} (h : ValidProgram (n +1) (pop :: p')) : ValidProgram n p' := by -- starts tactic mode; the goal extracts tail validity after a `pop`
  grind -- inverts the `pop` validity constructor and returns the proof for the remaining program

/-!
## Running of Valid Programs

We define a function that runs a program on a stack, given a proof that the program is valid for the initial stack size. -/

/--
Runs a program that is guaranteed to be valid for the initial stack size.
This function is total and does not need to return `Option`.
-/
def run (p: Program) (initStack: Stack) (h: ValidProgram initStack.length p) : Stack := -- defines `run`
  match p with -- splits computation into cases by pattern matching
  | [] => initStack -- matches the empty list and returns `initStack`
  | push n :: p' => -- matches a program beginning with `push n` and runs the rest with `n` pushed on the stack
      run p' (n :: initStack) (valid_program_of_push h)
  | add :: p' => -- matches a program beginning with `add` and checks the initial stack shape before adding the top two entries
      ...
      | x :: y :: zs => -- matches a stack with at least two entries and runs the rest with `x + y` pushed above `zs`
        run p' ((x + y) :: zs) (valid_program_of_add h)
...
#eval run [push 2, add] [] (by grind) -- runs this expression as a tutorial check
...
-/
theorem valid_iff_eval?_some (p: Program) (s: Stack) : ValidProgram s.length p ↔ (run? p s) ≠ none := by -- starts tactic mode; the goal is an iff between static validity and safe evaluation success
  ...
  · intro h -- in the forward direction, assumes validity and changes the goal to `eval? p s ≠ none`
    match p with -- splits computation into cases by pattern matching
    | [] => grind [run?] -- matches the empty list and asks `grind` to solve this case
    | push n :: p' => -- matches a program beginning with `push n` and proves success for the rest with the larger stack
        ...
        grind [run?] -- unfolds safe evaluation for `push` and uses `ih` to prove the result is not `none`
    | add :: p' => -- matches a program beginning with `add` and proves success after checking the stack has two entries
        ...
        | x :: y :: zs => -- matches a stack with at least two entries and proves success for the summed stack
           ...
           grind [run?] -- unfolds the successful `add` case and uses `ih` to prove evaluation does not fail
    | pop :: p' => -- matches a program beginning with `pop` and proves success after checking the stack is nonempty
        ...
        | x :: ys => -- matches a nonempty stack and proves success for the tail stack
            ...
            grind [run?] -- unfolds the successful `pop` case and uses `ih` to prove evaluation does not fail
  · intro h -- in the backward direction, assumes safe evaluation succeeds and changes the goal to `ValidProgram s.length p`
    ...
    | push n :: p' => -- matches a program beginning with `push n` and derives validity of the rest from safe evaluation
        ...
        grind [run?] -- unfolds the `push` evaluator case and uses `ih` to build `ValidProgram.push`
    | add :: p' => -- matches a program beginning with `add` and derives validity after checking the stack has two entries
        ...
        | x :: y :: zs => -- matches a stack with at least two entries and derives validity for the summed stack
           ...
           grind [run?] -- unfolds the successful `add` evaluator case and builds the matching validity proof
    | pop :: p' => -- matches a program beginning with `pop` and derives validity after checking the stack is nonempty
        ...
        | x :: ys => -- matches a nonempty stack and derives validity for the tail stack
            ...
            simp [run?] at h -- unfolds safe evaluation in the hypothesis, leaving success of the tail program
